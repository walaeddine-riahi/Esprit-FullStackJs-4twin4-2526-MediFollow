...
export async function generateVitalReport(vitalRecordId: string) {
  try {
    ...
    // Generate detailed nursing report
    const nursingReport = await generateNursingReport({
      patientName: `${patient.user.firstName} ${patient.user.lastName}`,
      age,
    ...
    const report = {
      ...
      report: {
        ...
        currentVitals,
        riskScore: riskAnalysis.success ? riskAnalysis.analysis?.riskScore : 50,
        riskLevel: riskAnalysis.success ? riskAnalysis.analysis?.riskLevel : "Unknown",
        trendIndicator: riskAnalysis.success ? riskAnalysis.analysis?.trendIndicator : "stable",
        concerns: riskAnalysis.success ? riskAnalysis.analysis?.concerns : [],
        recommendations: riskAnalysis.success ? riskAnalysis.analysis?.recommendations : [],
        summary: riskAnalysis.success ? riskAnalysis.analysis?.summary : "Analyse en cours...",
...
export async function generatePatientReport(patientId: string) {
  try {
    ...
    const criticalAlerts = patient.alerts.filter((a: any) => a.severity === "CRITICAL").length;
    const openAlerts = patient.alerts.filter((a: any) => a.status === "OPEN").length;
    ...
    const report = {
      ...
      report: {
        ...
        riskScore: riskAnalysis?.success ? riskAnalysis.analysis?.riskScore : null,
        riskLevel: riskAnalysis?.success ? riskAnalysis.analysis?.riskLevel : "Unknown",
        trendIndicator: riskAnalysis?.success ? riskAnalysis.analysis?.trendIndicator : "stable",
        concerns: riskAnalysis?.success ? riskAnalysis.analysis?.concerns : [],
        recommendations: riskAnalysis?.success ? riskAnalysis.analysis?.recommendations : [],
        summary: riskAnalysis?.success ? riskAnalysis.analysis?.summary : "Aucune donnée vitale récente disponible.",
        detailedReport: nursingReport?.success ? nursingReport.report : "",
        statistics: {
