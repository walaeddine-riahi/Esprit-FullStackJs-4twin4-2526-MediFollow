...
export async function GET() {
  try {
    ...
    const [
      ...
      prisma.alert.count({ where: { ...NON_DUMMY_ALERT_WHERE, severity: "CRITICAL" } }),
      prisma.alert.count({ where: { ...NON_DUMMY_ALERT_WHERE, status: "OPEN" } }),
      prisma.alert.count({ where: { ...NON_DUMMY_ALERT_WHERE, status: "RESOLVED" } }),
