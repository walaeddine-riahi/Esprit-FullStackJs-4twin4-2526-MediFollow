      ...
      <div>
        ...
        <div className="mb-12">
          ...
          <p className="mt-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em]">
            {loading
              ...
              : `${totalResults} entries identified in the database`}
          </p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-32 text-slate-300 dark:text-slate-700">
            ...
            <span className="text-[10px] font-black tracking-[0.5em] animate-pulse">
              SYNCHRONIZING...
            </span>
          </div>
        ) : error ? (
          <div className="py-20 text-center border-2 border-dashed border-red-100 dark:border-red-900/20 rounded-[3rem]">
            ...
            <p className="text-sm font-bold text-red-500 italic">
              Connection error with the secure server.
            </p>
          </div>
        ) : (
          <div className="space-y-16">
                ...
                <div className="grid gap-3">
                  {userResults.map((user) => (
                    ...
                    >
                      ...
                      <ChevronRight
                        size={16}
                        className="text-slate-200 dark:text-slate-700 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 translate-x-0 group-hover:translate-x-1 transition-all"
                      />
                    </Link>
                  ))}
                </div>
                ...
                <div className="grid gap-3">
                  {alertResults.map((alert) => (
                    ...
                    >
                      ...
                      <ChevronRight
                        size={16}
                        className="text-slate-200 dark:text-slate-700 group-hover:text-rose-600 dark:group-hover:text-rose-400 translate-x-0 group-hover:translate-x-1 transition-all"
                      />
                    </Link>
                  ))}
                </div>
            ...
            {totalResults === 0 && (
              <div className="py-32 text-center bg-slate-50 dark:bg-slate-900/50 rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800 transition-all">
                ...
                />
                <p className="text-lg font-bold text-slate-400 dark:text-slate-600 italic">
                  No matches found in the encrypted archives.
                </p>
                <button
                  onClick={() => router.back()}
