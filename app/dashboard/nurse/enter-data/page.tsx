...
import { parseVitalsFromVoice, generateVitalReport } from "@/lib/actions/ai.actions";
    ...
    if (parsed.success && parsed.vitals) {
      ...
      if (form) {
        if (parsed.vitals.systolicBP) {
          (form.querySelector('[name="systolicBP"]') as HTMLInputElement).value = parsed.vitals.systolicBP.toString();
        }
        if (parsed.vitals.diastolicBP) {
          (form.querySelector('[name="diastolicBP"]') as HTMLInputElement).value = parsed.vitals.diastolicBP.toString();
        }
        if (parsed.vitals.heartRate) {
          (form.querySelector('[name="heartRate"]') as HTMLInputElement).value = parsed.vitals.heartRate.toString();
        }
        if (parsed.vitals.temperature) {
          (form.querySelector('[name="temperature"]') as HTMLInputElement).value = parsed.vitals.temperature.toString();
        }
        if (parsed.vitals.oxygenSaturation) {
          (form.querySelector('[name="oxygenSaturation"]') as HTMLInputElement).value = parsed.vitals.oxygenSaturation.toString();
        }
        if (parsed.vitals.weight) {
          (form.querySelector('[name="weight"]') as HTMLInputElement).value = parsed.vitals.weight.toString();
        }
      ...
      {generatingReport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-lg p-6 text-center">
            ...
            <p className="text-gray-900 dark:text-white font-medium">Génération du rapport AI...</p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Analyse des données en cours</p>
          </div>
        </div>
      )}
    </div>
