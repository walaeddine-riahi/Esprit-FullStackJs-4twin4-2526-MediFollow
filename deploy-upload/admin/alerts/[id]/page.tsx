...
const IconGlyph = ({ className }: IconProps) => (
  <span aria-hidden className={className}>
    *
  </span>
);

const Clock = IconGlyph;
const CheckCircle = IconGlyph;
const ArrowLeft = IconGlyph;
const User = IconGlyph;
const Activity = IconGlyph;
const AlertTriangle = IconGlyph;
const Info = IconGlyph;
const Mail = IconGlyph;
const Phone = IconGlyph;
const Heart = IconGlyph;
const Thermometer = IconGlyph;
const Wind = IconGlyph;
const Gauge = IconGlyph;
const Sparkles = IconGlyph;
...
export default function AlertDetailsPage({
  ...
  const checkAuthAndLoadAlert = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/alerts/${encodeURIComponent(params.id)}`,
        { cache: "no-store" }
      );
      ...
      if (result.success && result.alert) {
        setAlert({
          ...
          createdAt: result.alert.createdAt
            ? new Date(result.alert.createdAt)
            : new Date(),
        ...
        setError(
          result?.detail || result?.error || "Unable to load the alert."
        );
      }
    } catch (err) {
  ...
  const loadAiActions = useCallback(async () => {
    try {
      ...
      setAiError(null);
      const response = await fetch(
        `/api/admin/alerts/${encodeURIComponent(params.id)}/next-actions`,
        { cache: "no-store" }
      );
      const result = await response.json();
      if (result.success) {
  ...
  const getSeverityStyles = (severity: string) => {
    ...
    > = {
      CRITICAL: {
        ...
        icon: (
          <Info size={24} className="text-red-600 dark:text-red-500" />
        ),
        label: "Critical",
      },
      HIGH: {
      ...
      MEDIUM: {
        ...
        icon: (
          <Info size={24} className="text-yellow-600 dark:text-yellow-500" />
        ),
        label: "Medium",
      },
      LOW: {
  ...
  const getStatusStyles = (status: string) => {
    const styles: Record<
      string,
      { color: string; icon: JSX.Element | null; label: string }
    > = {
      OPEN: {
        color:
  ...
  const getVitalConfig = (type: string) => {
    const config: Record<
      string,
      { icon: JSX.Element; label: string; unit: string }
    > = {
      systolicBP: {
        icon: <Gauge size={16} className="text-red-500" />,
    ...
    return (
      config[type] || { icon: <Activity size={16} />, label: type, unit: "" }
    );
  };
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900 overflow-hidden shadow-sm">
          <div className={`p-6 ${severity.color} border-b border-inherit`}>
            <div className="flex items-center gap-4">
              <div className="p-3 bg-white/50 dark:bg-slate-900/50 rounded-xl shadow-sm">
                {severity.icon}
              </div>
              <div>
          ...
          <div className="p-6 md:p-8">
            <div className="mb-8 flex flex-wrap items-center gap-4">
              <span
                className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold border tracking-wide uppercase ${status.color}`}
              >
            ...
            <div className="mb-10 p-5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
              <p className="text-gray-900 dark:text-slate-200 text-lg leading-relaxed font-medium">
                {alert.message}
              </p>
            </div>
            ...
            <div className="mb-10 rounded-xl border border-indigo-100 dark:border-indigo-900/40 bg-gradient-to-br from-indigo-50 to-cyan-50 dark:from-indigo-950/30 dark:to-cyan-950/30 p-5">
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-gray-600 dark:text-slate-400">
                    {aiSummary || "Top 3 recommended actions with confidence."}
                  </p>
                </div>
                <button
                  type="button"
              ...
              {aiLoading ? (
                <div className="py-4 text-sm text-indigo-700 dark:text-indigo-300">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {aiActions.map((item, idx) => (
                    <div
                      key={`${item.title}-${idx}`}
                      className="rounded-lg border border-white/60 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 p-4 shadow-sm"
                    >
                      <div className="mb-2 flex items-center justify-between gap-2">
                        <span className="text-[10px] font-black tracking-wider text-indigo-600 dark:text-indigo-300 uppercase">
                          Action {idx + 1}
                        </span>
                      ...
                      <p className="text-sm font-bold text-gray-900 dark:text-slate-100 mb-1.5">
                        {item.title}
                      </p>
            ...
            {alert.data?.vitalType && (
              <div className="mb-10">
                <h3 className="font-bold text-gray-900 dark:text-slate-100 mb-4 flex items-center gap-2">
                  <Activity size={18} className="text-blue-500" /> Vitals at the
                  time of alert
                </h3>
            ...
            <div className="mb-10">
              <h3 className="font-bold text-gray-900 dark:text-slate-100 mb-6">
                Action History
              </h3>
              <div className="space-y-8 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100 dark:before:bg-slate-800">
                <TimelineItem
                  icon={<Info size={14} className="text-orange-600" />}
                  color="bg-orange-100 dark:bg-orange-950/40"
                  title="Alert generated by the system"
            ...
            {alert.resolution && (
              <div className="mb-10 p-5 bg-green-50/50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 rounded-xl">
                ...
                <p className="text-green-800 dark:text-green-300 leading-relaxed italic">
                  "{alert.resolution}"
                </p>
              </div>
            )}
              ...
              <div className="border-t border-gray-100 dark:border-slate-800 pt-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <PatientInfoItem
                    ...
                    label="Full Name"
                    value={`${alert.patient.user.firstName} ${alert.patient.user.lastName}`}
                  ...
                  <PatientInfoItem
                    ...
                    label="Email"
                    value={alert.patient.user.email}
                  ...
                  {alert.patient.user.phoneNumber && (
                    <PatientInfoItem
                      ...
                      label="Contact"
                      value={alert.patient.user.phoneNumber}
...
function TimelineItem({
  ...
  return (
    <div className="flex items-start gap-4 relative z-10">
      <div
        className={`h-9 w-9 rounded-full ${color} flex items-center justify-center flex-shrink-0 shadow-sm border border-white dark:border-slate-900`}
      >
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-bold text-gray-900 dark:text-slate-200 text-sm leading-tight">
          {title}
        </p>
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm transition-all hover:border-blue-200 dark:hover:border-blue-900">
      ...
      <div className="min-w-0">
        <p className="text-[10px] uppercase tracking-widest text-gray-400 dark:text-slate-500 font-black mb-0.5">
          {label}
        </p>
