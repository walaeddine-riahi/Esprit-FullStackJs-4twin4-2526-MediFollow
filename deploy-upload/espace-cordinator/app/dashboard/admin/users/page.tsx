...
export default function AdminUsersPage() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      console.log("[AdminUsers] Data fetched:", data, "Length:", Array.isArray(data) ? data.length : "not an array");
      
      if (!Array.isArray(data)) {
        console.warn("[AdminUsers] getData did not return an array:", typeof data);
        setLoadError("Failed to load users: Invalid data format");
        setUsers([]);
        return;
      }
      ...
      if (data.length === 0) {
        ...
        setLoadError("No users found in database");
      }
      
      const mappedUsers = data.map((u: any) => {
        console.log("[AdminUsers] Mapping user:", u);
        return {
          ...u,
      ...
      console.error("[AdminUsers] Loading error:", err);
      setLoadError(`Failed to load users: ${err?.message || "Unknown error"}`);
      setUsers([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (created && !created.success) {
        alert("Failed to create user: " + created.error);
        return;
      }
  ...
  if (loadError) return (
    <div className="p-8 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
      <p className="text-red-800 dark:text-red-300 font-semibold">Error loading users:</p>
      <p className="text-red-700 dark:text-red-400 text-sm mt-2">{loadError}</p>
      <button 
        onClick={() => loadUsers()}
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-2xl font-black tracking-tight">Profile Management <span className="text-xs text-slate-500">({users.length} total, {filteredUsers.length} filtered)</span></h1>
        </div>
        <button 
