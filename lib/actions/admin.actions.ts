...
export async function getAllNurses() {
  try {
    ...
    return { success: false, error: "Erreur lors du chargement des infirmiers" };
  }
}
...
export async function createNurse(data: {
  ...
  try {
    ...
    return { success: false, error: "Erreur lors de la création de l'infirmier" };
  }
}
...
export async function updateNurse(
  ...
  try {
    ...
    if (nurse.nurseProfile && (data.department || data.shift || data.phoneNumber)) {
    ...
    return { success: false, error: "Erreur lors de la mise à jour de l'infirmier" };
  }
}
...
export async function deleteNurse(nurseId: string) {
  try {
    ...
    return { success: false, error: "Erreur lors de la suppression de l'infirmier" };
  }
}
...
export async function getAllCoordinators() {
  try {
    ...
    return { success: false, error: "Erreur lors du chargement des coordinateurs" };
  }
}
...
export async function createCoordinator(data: {
  ...
  try {
    ...
    return { success: false, error: "Erreur lors de la création du coordinateur" };
  }
}
...
export async function updateCoordinator(
  ...
  try {
    ...
    if (coordinator.coordinatorProfile && (data.department || data.phoneNumber)) {
      const profileData: any = {};
      if (data.department) profileData.department = data.department;
      if (data.phoneNumber) profileData.phone = data.phoneNumber;
...
export async function assignPatientToNurse(
  ...
  try {
    ...
    if (error.code === "P2002") {
      return { success: false, error: "Ce patient est déjà assigné à cet infirmier" };
    }
...
export async function unassignPatientFromNurse(
  ...
  try {
    ...
    return { success: false, error: "Erreur lors de la désassignation du patient" };
  }
}
...
export async function getNurseAssignments(nurseId: string) {
  try {
    ...
    return { success: false, error: "Erreur lors du chargement des assignations" };
  }
}
