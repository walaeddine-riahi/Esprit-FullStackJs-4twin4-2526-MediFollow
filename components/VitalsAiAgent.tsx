...
export default function VitalsAiAgent({
  vitals,
  patientContext,
}: VitalsAiAgentProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setReportError(
          data.error || "Une erreur s'est produite lors de la génération."
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: "Désolé, impossible de répondre pour le moment.",
          },
        ]);
      }
    } catch (err) {
        ...
        {generatingReport && (
          <div className="flex flex-col items-center justify-center py-10 gap-4">
            ...
            <p className="text-sm font-medium text-gray-500">
              Lecture des lignes et analyse des tendances en cours...
            </p>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
              <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 mb-4">
                <AlertTriangle className="size-4 text-amber-500" /> Analyse par
                paramètre (Bruit & Valeurs Sûres)
              </h3>
              <div className="space-y-3">
                {(report.analyseParParametre || []).map(
                  (item: any, i: number) => (
                    <div
                      key={i}
                      className="flex flex-col sm:flex-row gap-2 sm:gap-6 pb-3 border-b border-gray-100 dark:border-gray-800 last:border-0 last:pb-0"
                    >
            ...
            <div className="pt-2">
                ...
                {chatLoading && (
                  <div className="flex justify-start">
                    <div className="bg-gray-100 dark:bg-gray-800 text-gray-500 rounded-2xl rounded-bl-none px-4 py-3 flex items-center gap-2">
                      <Loader2 className="size-4 animate-spin" /> L'agent
                      tape...
                    </div>
                  </div>
                )}
              ...
              <form
                onSubmit={handleChat}
                className="flex items-center gap-2 bg-gray-50 dark:bg-gray-900 rounded-full border border-gray-200 dark:border-gray-800 p-1"
              >
                <input
                  type="text"
                  value={chatInput}
