...
export default function ServiceDetailsPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (serviceRes.success) setService(serviceRes.service as Service);
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/dashboard/admin/services" className="text-slate-500 hover:text-slate-300 transition-colors">
              <ArrowLeft size={20} />
            </Link>
            <div>
              <h1 className="text-2xl font-bold">{service.serviceName}</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">Service details and assignments.</p>
            </div>
          </div>
          <Link
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 lg:col-span-1">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div>
                <p className="font-semibold text-slate-800 dark:text-slate-200">Specializations:</p>
                {service.specializations.length === 0 ? (
                  <p className="text-slate-500">None</p>
                ) : (
                  <div className="mt-1 flex flex-wrap gap-2">
                    {service.specializations.map((item) => (
                      <span key={item} className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                        {item}
                      </span>
                    ))}
                  </div>
            ...
            {assignedPatients.length === 0 ? (
              ...
              <ul className="space-y-2">
                {assignedPatients.map((patient) => (
                  <li key={patient.id} className="rounded border border-slate-200 p-2 text-sm dark:border-slate-700">
                    <p className="font-medium text-slate-900 dark:text-white">{patient.label}</p>
            ...
            {assignedTeam.length === 0 ? (
              ...
              <ul className="space-y-2">
                {assignedTeam.map((member) => (
                  <li key={member.id} className="rounded border border-slate-200 p-2 text-sm dark:border-slate-700">
                    <p className="font-medium text-slate-900 dark:text-white">{member.label}</p>
