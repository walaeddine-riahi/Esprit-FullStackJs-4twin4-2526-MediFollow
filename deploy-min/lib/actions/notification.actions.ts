...
export async function sendEmail(to: string, subject: string, html: string) {
  try {
    ...
    if (process.env.GMAIL_EMAIL && process.env.GMAIL_PASSWORD) {
      try {
        ...
        const result = await transporter.sendMail({
          from: `"MediFollow" <${process.env.GMAIL_EMAIL}>`,
          to,
          subject,
          html,
        });

        console.log(`✅ Email envoyé à ${to} (via Gmail)`);
        return { success: true, messageId: result.messageId };
      } catch (gmailError) {
        console.warn(
          `⚠️  Gmail SMTP failed: ${gmailError instanceof Error ? gmailError.message : String(gmailError)}`
        );
      }
    }
    ...
    // Fallback to generic SMTP if Resend not configured
    ...
    console.warn(
      "[Notification] No Email provider configured (missing GMAIL_EMAIL/GMAIL_PASSWORD, RESEND_API_KEY and SMTP credentials), skipping email to",
      to
    );
...
export async function sendStaffCredentialsEmail(
  ...
  try {
    ...
    // Try Gmail SMTP first (Nodemailer)
    let emailError: string | null = null;

    // Try Gmail configuration (GMAIL_EMAIL and GMAIL_PASSWORD)
    if (process.env.GMAIL_EMAIL && process.env.GMAIL_PASSWORD) {
      try {
        console.log(
          `📧 Attempting to send via Gmail SMTP: ${process.env.GMAIL_EMAIL}`
        );
        const transporter = nodemailer.createTransport({
          host: "smtp.gmail.com",
          port: 587,
        ...
        console.log(`✅ Email credentials sent via Gmail SMTP to ${email}`);
        return { success: true, messageId: result.messageId };
      } catch (error) {
        emailError = error instanceof Error ? error.message : String(error);
        console.warn(`⚠️  Gmail SMTP failed: ${emailError}`);
      }
    }
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
      try {
        ...
        emailError = error instanceof Error ? error.message : String(error);
        console.warn(`⚠️  SMTP failed: ${emailError}`);
      }
    }
    ...
    const errorSummary = emailError
      ? `Email service error: ${emailError}`
    ...
    return {
      success: false,
      error: errorSummary,
      hint: "Configure Gmail (GMAIL_EMAIL and GMAIL_PASSWORD with app password), or SMTP credentials, or RESEND_API_KEY",
    };
  } catch (error) {
