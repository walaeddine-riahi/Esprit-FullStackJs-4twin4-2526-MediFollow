...
export default async function AdminDashboard() {
  ...
  const alertData = alertRes?.stats ?? { total: 0, critical: 0, open: 0, resolved: 0 };
  ...
  return (
    <>
      <div className="mb-6 grid grid-cols-2 gap-3 lg:hidden">
        <Link href="/dashboard/admin/alerts" className="glass-panel rounded-2xl border border-slate-200 dark:border-cyan-300/25 p-3 text-sm font-bold text-slate-700 dark:text-cyan-100">
          Incidents
        </Link>
        <Link href="/dashboard/admin/users" className="glass-panel rounded-2xl border border-slate-200 dark:border-cyan-300/25 p-3 text-sm font-bold text-slate-700 dark:text-cyan-100">
          Users
        </Link>
