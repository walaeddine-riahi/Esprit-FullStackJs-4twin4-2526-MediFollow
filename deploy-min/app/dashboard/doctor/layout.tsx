...
function DoctorLayoutInner({ children }: { children: ReactNode }) {
  ...
  async function loadData() {
    try {
      ...
      const [alertsRes, statsRes, profileRes] = await Promise.all([
        getAllAlerts(AlertStatus.OPEN),
        getAlertStats(),
        getDoctorProfile(currentUser.id),
      ]);
      if (alertsRes.success)
        setRecentAlerts((alertsRes.alerts ?? []).slice(0, 10));
      if (statsRes.success) setOpenAlertsCount(statsRes.stats?.critical || 0);
      if (profileRes.success && (profileRes as any).data?.profileImage) {
        setProfileImage((profileRes as any).data.profileImage);
      }
      if (profileRes.success && (profileRes as any).data?.specialty) {
        setDoctorSpecialty((profileRes as any).data.specialty);
      }
    } catch {}
  }
  ...
  function handleSearch(e: React.FormEvent) {
    ...
    if (searchQuery.trim()) {
      router.push(
        `/dashboard/doctor/patients?search=${encodeURIComponent(searchQuery.trim())}`
      );
      setSearchQuery("");
    }
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white dark:bg-black transition-colors">
      {/* Sidebar - Kick Style (Deep Dark & Green Accents) */}
      <aside
      ...
      >
        {/* Logo - Style MediFollow avec gradient vert (Kick) */}
        <div className="flex h-20 items-center justify-between border-b border-gray-100 dark:border-gray-800 px-5">
          ...
          >
            <div className="relative">
              <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-green-600 to-green-700 dark:from-white dark:to-gray-100 opacity-0 blur-lg transition-opacity group-hover:opacity-50"></div>
              <div className="relative flex items-center justify-center rounded-xl bg-gradient-to-br from-green-600 via-green-700 to-green-700 p-2 shadow-lg shadow-green-500/30 dark:shadow-white/30 transition-all duration-300 group-hover:shadow-xl group-hover:shadow-green-500/50 dark:group-hover:shadow-white/50">
                <Activity className="size-7 text-white animate-pulse" />
              </div>
            </div>
            <div className="flex flex-col">
              <span className="text-xl font-black leading-none bg-gradient-to-r from-green-600 via-green-700 to-green-700 bg-clip-text text-transparent">
                MediFollow
              </span>
              ...
              return (
                <Link
                  ...
                  className={`group relative flex items-center gap-4 rounded-xl px-4 py-3 text-sm font-medium transition-all ${
                    isActive
                      ? "bg-gradient-to-r from-green-500/10 to-green-400/10 dark:from-green-500/20 dark:to-green-400/20 text-green-600 dark:text-green-400 shadow-sm"
                      : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50"
                  }`}
                >
                  {isActive && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-1 bg-gradient-to-b from-green-500 to-green-400 rounded-r-full"></div>
                  )}
                  <Icon className="size-5 flex-shrink-0" />
                  <span className="flex-1">{item.label}</span>
                  {item.badge !== undefined && item.badge > 0 && (
                    <span className="rounded-full bg-gradient-to-r from-green-500 to-green-600 dark:from-green-500 dark:to-green-600 px-2 py-0.5 text-xs font-bold text-black dark:text-black shadow-sm dark:shadow-green-500/50 animate-pulse">
                      {item.badge}
                    </span>
                  )}
                </Link>
        ...
        {/* User Profile - Carte avec fond dégradé */}
        <div className="border-t border-gray-100 dark:border-gray-800 p-3">
          <Link
            href="/dashboard/doctor/profile"
            className="block relative rounded-xl bg-gradient-to-br from-green-600 to-green-700 dark:from-green-600/90 dark:to-green-700/90 p-4 shadow-lg overflow-hidden hover:shadow-xl transition-shadow cursor-pointer border dark:border-green-500/20"
          >
            ...
            <div className="relative flex items-center gap-3">
              {/* Avatar avec icône stéthoscope ou photo */}
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                        ...
                        {recentAlerts.length === 0 ? (
                          ...
                          recentAlerts.map((alert: any) => {
                            ...
                            return (
                              ...
                              >
                                <div className="size-9 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-black font-bold text-sm flex-shrink-0">
                ...
                {user?.blockchainAddress && (
                  <div className="hidden xl:flex items-center gap-2 rounded-lg bg-green-50 dark:bg-green-500/10 px-3 py-2 border border-green-200 dark:border-green-500/20">
                    <div className="size-2 rounded-full bg-green-600 dark:bg-green-400 animate-pulse" />
                    <span className="text-xs font-medium text-green-900 dark:text-green-400">
                      Blockchain Active
                    </span>
                  </div>
                )}
