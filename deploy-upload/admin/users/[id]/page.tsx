...
export default function UserDetailsPage({
  ...
  const RoleBadge = ({ role }: { role: string }) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ring-1 ring-inset ${color}`}
      >
  ...
  if (!user)
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <Link
          href="/admin/users"
          className="mt-4 text-sm font-bold text-indigo-500 hover:underline"
        >
          Back to list
        </Link>
      </div>
  ...
  return (
    <div className="pb-20">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-[10px] font-black uppercase tracking-widest text-slate-400 leading-none mb-1">
            Profile Details
          </h1>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[32px] p-8 text-center shadow-sm relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <h2 className="text-xl font-black text-slate-900 dark:text-white mt-4 leading-tight">
                  {user.name}
                </h2>
                <p className="text-sm font-medium text-slate-400 dark:text-slate-500 break-all">
                  {user.email}
                </p>
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-slate-50 dark:border-slate-800">
                <div className="text-center">
                  ...
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-full">
                    <div
                      className={`size-2 rounded-full ${user.isActive ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
                    />
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl p-10 text-center animate-in zoom-in duration-300 border border-slate-100 dark:border-slate-800">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-8 px-4">
              Warning, this action is irreversible. The data of{" "}
