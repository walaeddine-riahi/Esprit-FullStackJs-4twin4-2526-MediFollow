...
async function main() {
  ...
  await prisma.coordinatorEntryFlag.deleteMany();
  await prisma.coordinatorReminder.deleteMany();
  await prisma.coordinatorPatient.deleteMany();
  ...
  await prisma.medicalAnalysis.deleteMany();
  await prisma.patientCheckup.deleteMany();
  ...
  // Coordinator — suivi protocole post-hospitalisation
  console.log("📋 Creating coordinator user...");
  const coordinator = await prisma.user.create({
    data: {
      email: "coordinator@medifollow.health",
      passwordHash: hashedPassword,
      firstName: "Samira",
      lastName: "Benali",
      role: "COORDINATOR",
      phoneNumber: "+33612345677",
      isActive: true,
    },
  });
  ...
  // Assign all demo patients to coordinator
  console.log("🔗 Linking patients to coordinator...");
  await prisma.coordinatorPatient.createMany({
    data: [
      {
        coordinatorId: coordinator.id,
        patientId: patient1.id,
        notes: "Suivi principal post-sortie cardiologie",
      },
      {
        coordinatorId: coordinator.id,
  ...
  await prisma.questionnaire.createMany({
    data: [
      {
        patientId: patient1.id,
        questionnaireType: "suivi_hebdomadaire",
        responses: { q1: "ok", fatigue: "2" },
        overallScore: 82,
  ...
  console.log("│   Email: coordinator@medifollow.health               │");
  console.log("│   Password: Password123!                            │");
  console.log("└─────────────────────────────────────────────────────┘");
  console.log("\n📊 Data Summary:");
  console.log(`  - 6 Users (1 Admin, 1 Doctor, 1 Coordinator, 3 Patients)`);
